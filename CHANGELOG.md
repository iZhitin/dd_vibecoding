...
### Добавлено
- **[OPS-099]** Уборка этапа OPS.
  - Настроена маскировка сенситивных ключей и полных email-адресов в модуле структурированного логирования (`structlog`).
  - Добавлена функция `before_send` в фильтры Sentry для исключения секретных данных из payload перед отправкой.
  - Актуализирован `docker-compose.yml`: добавлены блоки ограничений ресурсов (`deploy.resources.limits` и `deploy.resources.reservations`) всем контейнерам.
  - Установлены права на исполнение (`+x`) для скрипта `scripts/backup.sh`.
  - Успешно пройдена финальная проверка `ruff check` (backend) и `npm run lint`, `npx prettier --write` (frontend).
- **[OPS-004]** Настроены HTTPS и email-аутентификация домена.
  - Обновлена конфигурация `nginx/nginx.conf`: добавлен редирект с HTTP на HTTPS и настройка SSL-серверов.
  - Добавлен контейнер `certbot` в `docker-compose.yml` для автоматического продления сертификатов.
  - Создан `README.md` с описанием шагов по настройке DNS-записей (A, DKIM, SPF, DMARC) и первоначальному получению сертификатов.
...
- **[OPS-002]** Интегрирован Sentry для отслеживания ошибок.
  ...
  - Добавлена инициализация Sentry в FastAPI приложении для окружений, отличных от `development`.
- **[OPS-001]** Настроено использование `structlog` для структурированного логирования.
  - Установлен и интегрирован `structlog` вместо стандартного логгера FastAPI/Uvicorn.
  - Настроен вывод JSON для production окружений и цветной лог для development.
  - Добавлен `logging_middleware` для генерации и сквозной передачи `request_id` через `structlog.contextvars`.
- **[SEC-099]** Уборка этапа SEC. Проверен маскинг секретов в Docker, созданы исключения `.dockerignore` и верифицировано отсутствие IDOR (Insecure Direct Object Reference).
- **[SEC-003]** В NGINX и FastAPI добавлены HTTP Security Headers: CSP, XSS-Protection, X-Content-Type-Options, Referrer-Policy, X-Frame-Options. CORS настроен на APP_URL.
- **[SEC-002]** Интегрирован SlowAPI для защиты от Brute-Force и L7 DDoS: добавлены лимиты 5/minute для Magic tokens, 20 req/min для `/api/translate`, и 30 r/s в NGINX. 
- **[SEC-001]** Реализована XSS-санитизация: создана утилита для экранирования тегов `<` и `>`, валидаторы Pydantic ограничивают `max_length` строк и автоматически очищают пользовательские ответы.
- **[TEST-099]** Проведена уборка этапа TEST.
  - Проверена полная изоляция тестов, прогнан `pytest` без выявления flaky-тестов.
  - Замоканы все запросы к внешним API, тесты полностью независимы и не используют реальных ключей.
  - Устранено дублирование в фикстурах и исправлены ошибки линтера `ruff` в `test_email.py`, `test_cards.py`, `test_llm_review.py`, `conftest.py`, `factories.py`.
  - Успешно пройдена проверка `ruff check` для всех тестов, все 45 тестов стабильно зеленые.
- **[TEST-004]** Написан тест LLM Worker с мок-ответами (`test_llm_review.py`).
  - Созданы фикстуры для мока базы данных и клиента OpenAI.
  - Успешно проверен сценарий корректного ответа (заполняются `grade` и `llm_feedback`).
  - Валидирован graceful degradation при получении некорректного JSON ответа (отлов ошибки без креша воркера, retry 3 раза).
  - Сценарий таймаутов API корректно перехватывается, воркер совершает 3 попытки перед завершением работы.
  - Написаны проверки автоматического обновления весов (`weight`) карточек для SRS-алгоритма после ответа LLM.
- **[TEST-003]** Написаны тесты API-эндпоинтов (Auth, Cards, Practice).
  - В `test_practice.py` добавлены проверки 403 ошибки при отправке чужой сессии и 422 ошибки при передаче менее 10 предложений.
  - В `test_cards.py` добавлена проверка изоляции выдачи карточек, изолированная по пользователям.
  - Удостоверен проход всех тестов `test_auth.py` (включая авторизацию, генерацию сессий и проверку Magic Link).
