...
- [ ] [INIT]-[006] Настроить линтеры и форматтеры
...
- [ ] [DATA]-[005] Создать seed-данные для разработки
  ...
  **Критерии готовности (DoD):**
  - [ ] `python -m scripts.seed` заполняет БД тестовыми данными без ошибок
  - [ ] Повторный запуск не создаёт дубликатов (проверка по email)
  - [ ] В БД есть пользователь, 15 карточек, 2 сессии, 20 логов

---

- [ ] [DATA]-[099] Уборка этапа DATA
  ...
  **Критерии готовности (DoD):**
  - [ ] Единообразное именование во всех моделях и схемах
  - [ ] Нет дублирования полей/определений
  - [ ] `ruff check` проходит без предупреждений на models/ и schemas/
...
- [ ] [CORE]-[001] Реализовать POST /api/cards — Capture (ввод нового слова)
  ...
  **Критерии готовности (DoD):**
  - [ ] `POST /api/cards` с `{"word": "serendipity", "translation": "счастливая случайность"}` возвращает 201 и JSON карточки
  - [ ] `GET /api/cards` возвращает список карточек текущего пользователя
  - [ ] Карточки одного пользователя не видны другому
...
- [ ] [CORE]-[003] Реализовать SRS-алгоритм (вероятностная выборка)
  ...
  **Критерии готовности (DoD):**
  - [ ] `select_practice_cards` возвращает 10 карточек (или меньше, если в базе мало)
  - [ ] Карточки с большим весом выбираются чаще (статистическая проверка: при 1000 выборок)
  - [ ] `update_weight_after_review` корректно обновляет вес для всех 4 грейдов
  - [ ] Вес никогда не падает ниже 0.01

---

- [ ] [CORE]-[004] Реализовать GET /api/practice/daily — генерация сессии
  ...
  **Критерии готовности (DoD):**
  - [ ] `GET /api/practice/daily` возвращает `session_id` + список из 10 карточек
  - [ ] Каждая карточка содержит `card_id`, `word`, `context_sentence`, `previous_sentence` (nullable)
  - [ ] Перевод НЕ включён в ответ (скрыт под кнопкой на UI)
  - [ ] Повторный вызов при активной сессии возвращает ту же сессию
...
- [ ] [CORE]-[007] Реализовать Streak-логику
  ...
  **Критерии готовности (DoD):**
  - [ ] Практика каждый день подряд → streak растёт (1, 2, 3, ...)
  - [ ] Пропуск дня → streak = 1
  - [ ] Заморозка: при `is_frozen=True, streak_frozen_count=1` пропуск не обнуляет streak
  - [ ] Две практики в один день → streak не меняется (не +2)

---

- [ ] [CORE]-[099] Уборка этапа CORE
  ...
  **Критерии готовности (DoD):**
  - [ ] Нет N+1 запросов в API-эндпоинтах
  - [ ] SRS-коэффициенты вынесены в именованные константы
  - [ ] `submit_practice` атомарный (транзакция)
  - [ ] `ruff check` проходит без предупреждений
...
- [ ] [NOTIFY]-[002] Создать email-шаблоны (Magic Link, Reminder, Daily Digest)
  ...
  **Критерии готовности (DoD):**
  - [ ] 3 HTML-шаблона создано
  - [ ] Шаблоны визуально корректны (проверить, открыв HTML в браузере)
  - [ ] Функции-обёртки принимают нужные параметры и формируют корректный HTML

---

- [ ] [NOTIFY]-[003] Реализовать Smart Nudge (Reminder)
  ...
  **Критерии готовности (DoD):**
  - [ ] Cron-задача `smart_nudge_check` запускается каждый час
  - [ ] Пользователь получает reminder, если не практиковался и прошёл час после среднего времени
  - [ ] Повторный reminder в тот же день не отправляется
  - [ ] `avg_practice_time` обновляется при каждой практике
...
- [ ] [UI]-[001] Создать Layout и систему роутинга с auth-guard
  ...
  **Критерии готовности (DoD):**
  ...
  - [ ] Layout отображается на всех защищённых страницах
  - [ ] 401 от API приводит к logout и redirect на `/login`
...
- [ ] [UI]-[003] Создать страницу Capture (ввод нового слова)
  ...
  **Критерии готовности (DoD):**
  - [ ] Ввод слова → loading skeleton → перевод появляется
  - [ ] Поля «слово» и «перевод» редактируемы
  - [ ] «Save» сохраняет карточку и очищает форму
  - [ ] При ошибке API перевода — пустое поле для ручного ввода
  - [ ] Auto-focus на поле ввода при загрузке страницы
...
- [ ] [UI]-[005] Создать страницу History (список слов и статистика)
  ...
  **Критерии готовности (DoD):**
  - [ ] Страница показывает список всех карточек пользователя
  - [ ] Streak отображается крупно
  - [ ] Работает сортировка по дате и по весу
  - [ ] Пагинация загружает следующие карточки

---

- [ ] [UI]-[006] Создать страницу Review (детальный разбор)
  ...
  **Критерии готовности (DoD):**
  - [ ] Страница загружает и отображает результаты сессии
  - [ ] Каждый лог показан с цветовой индикацией (Traffic Light)
  - [ ] GREEN_STAR выделен похвалой
  - [ ] YELLOW/RED показывают исправления
...
- [ ] [OPS]-[099] Уборка этапа OPS
  ...
  **Критерии готовности (DoD):**
  ...
  - [ ] Sentry фильтрует чувствительные данные
  - [ ] Docker-сервисы имеют restart policy и health checks
  - [ ] Финальный lint чистый по всему проекту
